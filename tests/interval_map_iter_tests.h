#ifndef INTERVAL_MAP_ITER_TESTS_H
...
static void test_im_int_iter_next_traversal(struct cvxtest *t)
{
    ...
    CVXCHECK(t, im_int_iter_value_hi(iter) == 25);
...
static struct cvxresult run_interval_map_iter_tests(void)
{
    ...
    printf("interval_map (iter)\n");

    CVXRUN(&t, test_im_int_iter_init_start);
    CVXRUN(&t, test_im_int_iter_init_start_empty);
    ...
    CVXRUN(&t, test_im_int_iter_next_traversal);
    CVXRUN(&t, test_im_int_iter_next_at_end);

    CVXRUN(&t, test_im_int_iter_prev_traversal);
    CVXRUN(&t, test_im_int_iter_prev_at_start);

    CVXRUN(&t, test_im_int_iter_forward);
    CVXRUN(&t, test_im_int_iter_forward_clamps);
...
#endif /* INTERVAL_MAP_ITER_TESTS_H */
